...
local function tick()
	...
	---@diagnostic disable-next-line: assign-type-mismatch
	primaryGaze.headRot.new = math.lerp(
		primaryGaze.headRot.old,
		primaryGaze.headRot.target,
...
---@field blink FOXGaze.Blink The blink animation to blend into
---@field package lerp {old: Vector2, new: Vector2}
---@field package blend {length: integer, target: integer}
---@field package parent FOXGaze
local anim = {}
...
function anim:render(delta)
	...
	if self.blink and self.blend.target then
		...
		self.horizontal:blend(a)
		self.vertical:blend(a)
		self.blink.animation:blend(b)

		if x >= 1 then self.blend.target = nil end
	end
...
---@param blink FOXGaze.Blink? `nil` The blink animation to blend into
---@return FOXGaze.Animation
function gaze:newAnim(horizontal, vertical, dampen, blink)
